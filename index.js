...
getName(makePersonObject(1, "Leia", "leia@leia.com"));
...
function appleIndex(slot) {
  return slot.indexOf("apple");
}
...
const lalal = require("./data/inventory");

function getCarInfoByIndex(inventory, index) {
  let first = inventory[index];
  return `The is a ${first.car_make} ${first.car_model}`;
}

getCarInfoByIndex(lalal, 0);
 ...
 */
function getLastCarInfo(Arg) {
  let lastCar = Arg[Arg.length - 1];

  return `This is a ${lastCar.car_make} ${lastCar.car_model}`;
}

getLastCarInfo(lalal);
/**
 ...
 */
function getModelYears(Arg) {
  let newArray = [];

  for (let i = 0; i < Arg.length; i++) {
    newArray.push(Arg[i].car_year);
  }
